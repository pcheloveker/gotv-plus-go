...
func (m *MatchesEngine) GetMatchesShort() ([]matchData, error) {
	if m.Matches == nil {
		return nil, fmt.Errorf("m.Matches == nil")
	}
	m.Lock()
	...
	mshort := make([]matchData, 0, len(m.Matches))
	for _, v := range m.Matches {
		mshort = append(mshort, matchData{ID: v.ID, Token: v.Token, Map: v.Map})
	}
	return mshort, nil
}
